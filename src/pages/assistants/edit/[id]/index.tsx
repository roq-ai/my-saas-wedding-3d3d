...
import { getAssistantById, updateAssistantById } from 'apiSdk/assistants';
...
function AssistantEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AssistantInterface>(
    () => (id ? `/assistants/${id}` : null),
    () => getAssistantById(id),
  );
  ...
  const handleSubmit = async (values: AssistantInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAssistantById(id, values);
      mutate(updated);
      resetForm();
